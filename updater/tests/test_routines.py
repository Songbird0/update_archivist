...
class IdenticalSumTest(django.test.TestCase):
    ...
    def setUp(self):
        updater.models.ScaffoldingState.objects.create(zipped_scaffolding_sum=self.SUM)
    ...
    def test_against_identical_sum(self):
        ...
        self.assertTrue(updater.models.ScaffoldingState.objects.filter(zipped_scaffolding_sum=zip_file_sum).exists(), 'The last scaffolding sum isn\'t equal to \n{0} but instead \n{1}'.format(zip_file_sum, self.SUM))
